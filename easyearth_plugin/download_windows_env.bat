...
REM usebackq is needed so the quoted python path is executed rather than treated
REM as a literal string, and the output is one token so delims must be cleared.
for /f "usebackq delims=" %%V in (`"%PYTHON_PATH%" -c "import sys; print(str(sys.version_info.major)+str(sys.version_info.minor))"`) do (
